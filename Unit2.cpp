...
// Преобразование текста (pdu и  из него)
String TForm1::workText(String text, String type){
	...
	if(type == "uncode"){
		...
		for(int i = 1; i <= text.Length()-2; i+=4){
			textConv = (String)"0x" + (String)text[i] + (String)text[i+1] + (String)text[i+2] + (String)text[i+3];
			textRes = textRes + wchar_t( StrToInt(textConv) );
		}
...
void __fastcall TForm1::Button2Click(TObject *Sender)
{
    ...
    // Получаем нормальный номер и вставляем его
	Edit4->Text = workText(pduFull, "decode");
}
//---------------------------------------------------------------------------
